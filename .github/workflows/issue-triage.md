...
Apply `infra-request` (in addition to a type/priority label, not instead of
one) when the issue describes Azure infrastructure or deployment needs —
e.g. provisioning resources, networking, environment configuration, or
changes under `/infrastructure`. This routes the issue toward the
Terraform custom agents (`.github/agents/`) once it is broken down via
`/plan`.
